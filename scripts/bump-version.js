...
if (!bumpType) {
  logError(
    'Please specify a bump type ("patch", "minor", "major") or a specific version (e.g. "1.1.0").'
  );
  process.exit(1);
}
...
if (['patch', 'minor', 'major'].includes(bumpType)) {
  ...
  if (!semverRegex.test(bumpType)) {
    logError(
      `Provided version "${bumpType}" is not a valid semver string or action (patch, minor, major).`
    );
    process.exit(1);
  }
  newVersion = bumpType;
...
if (fs.existsSync(buildGradlePath)) {
  ...
  if (gradleContent !== prevGradleContent) {
    ...
    logSuccess(
      `Updated android/app/build.gradle (versionCode: ${newVersionCode}, versionName: "${newVersion}")`
    );
  } else {
...
if (fs.existsSync(infoPlistPath)) {
  ...
  if (plistContent !== prevPlistContent) {
    ...
    logSuccess(
      `Updated ios/NeuroPilot/Info.plist (CFBundleShortVersionString: "${newVersion}", CFBundleVersion: "${newBuildNumber}")`
    );
  } else {
...
if (fs.existsSync(projectPbxprojPath)) {
  ...
  if (pbxprojContent !== prevPbxprojContent) {
    ...
    logSuccess(
      `Updated ios/NeuroPilot.xcodeproj/project.pbxproj (MARKETING_VERSION: ${newVersion}, CURRENT_PROJECT_VERSION: ${newBuildNumber})`
    );
  } else {
